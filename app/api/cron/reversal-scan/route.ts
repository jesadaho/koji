...
import { runCandleReversal1dAlertTick } from "@/src/candleReversal1dAlertTick";
...
/**
 * Manual cron — สแกน Reversal 1D (Binance) + อัปเดตสถิติ follow-up
...
export async function GET(req: NextRequest) {
  ...
  try {
    const now = Date.now();
    const notified = await runCandleReversal1dAlertTick(now);
